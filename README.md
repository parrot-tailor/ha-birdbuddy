...
| `media`       | The cover media (`{ "content_url", "thumbnail_url", ... }`), or `null`.                              |

`media.content_url` and `media.thumbnail_url` are time-sensitive URLs
that can be used to download the postcard image.

The payload is intentionally slim — it carries only what the collect
service and the Recent Visitor sensor need, not the full identification
report, so it stays well under Home Assistant's event-size limit.
...
## Breaking changes (v0.0.24 → v0.1.0)

This release adopts [pybirdbuddy] v0.1.0, which moved to Bird Buddy's
server-side `postcardCollect` flow. If you have automations that collect
postcards, you must update them:

- **The postcard event was renamed** from
  `birdbuddy_new_postcard_sighting` to
  [`birdbuddy_new_postcard`](#birdbuddy_new_postcard), and its payload
  changed. It no longer carries the `postcard` and `sighting` report
  objects; it now carries `postcard_id`, `feeder_id`, `species`, and
  `media`. (The slim payload also keeps the event under Home Assistant's
  event-size limit.)
- **The `birdbuddy.collect_postcard` service changed.** It now takes a
  `postcard_id` (plus optional `feeder_id` and `share`) instead of the
  `postcard` and `sighting` objects. The `strategy` and
  `best_guess_confidence` options are **gone** — Bird Buddy identifies
  species server-side, so the client no longer picks one — and
  `share_media` is now simply `share`.
- **Re-import the Blueprint.** The bundled [Blueprint](#blueprint) was
  updated for the renamed event and the new service inputs; re-import it
  and re-create any automation built from it.
- **Power Profile is now enabled by default** and adds the
  `Ultra Frenzy Mode` option.

If you use the Blueprint (or the Device Trigger with the service
pass-through above) without customizing the payload, re-importing the
Blueprint is all you need.
...
[build-status]: https://github.com/jhansche/ha-birdbuddy/actions/workflows/ci.yml?query=branch%3Amain
[build-status-shield]: https://img.shields.io/github/actions/workflow/status/jhansche/ha-birdbuddy/ci.yml?branch=main&style=for-the-badge
