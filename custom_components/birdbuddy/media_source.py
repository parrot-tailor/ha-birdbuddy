...
from homeassistant.components.media_source.error import (
    MediaSourceError,
    Unresolvable,
)
...
class BirdBuddyMediaSource(MediaSource):
    ...
    async def async_resolve_media(self, item: MediaSourceItem) -> PlayMedia:
        ...
        Raises:
            Unresolvable: If the identifier is incomplete or the media has
                no content URL.
        """
        config_id, collection_id, media_id = self._parse_identifier(
            item.identifier
        )
    ...
    async def async_browse_media(
        ...
        if item.identifier:
            ...
            config_id, collection_id, _ = self._parse_identifier(
                item.identifier
            )
            if config_id:
    ...
    async def _build_media_collection_entries(
        ...
        for media_id, media in medias.items():
            relative_title = _best_timedelta_title(
                media.created_at, dt_util.utcnow()
            )
            base.children.append(
                BrowseMediaSource(
                    ...
                    identifier=(
                        f"{config.entry_id}#"
                        f"{collection.collection_id}#{media_id}"
                    ),
