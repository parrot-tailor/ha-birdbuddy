...
async def async_setup_entry(
    ...
    """
    coordinator: BirdBuddyDataUpdateCoordinator = hass.data[DOMAIN][
        entry.entry_id
    ]
    ...
    async_add_entities(
        BirdBuddyRecentVisitorEntity(f, coordinator) for f in feeders
    )
    # Incubating: Food level always reports LOW
    async_add_entities(
        BirdBuddyFoodStateEntity(f, coordinator) for f in feeders
    )
    # Incubating: Temperature always reports 0
    async_add_entities(
        BirdBuddyTemperatureEntity(f, coordinator) for f in feeders
    )
    ...
    def entity_picture(self) -> str | None:
        ...
        if self._latest_media:
            picture = (
                ...
                or self._latest_media.thumbnail_url
            )
            if not is_media_expired(picture):
                return picture
