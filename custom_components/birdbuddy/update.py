...
class BirdBuddyUpdate(BirdBuddyMixin, UpdateEntity):
    ...
    def _raise_if_not_ready(self) -> None:
        ...
        if self.feeder.state in REJECT_STATES:
            msg = (
                "Cannot perform update when in state "
                f"{self.feeder.state.value}"
            )
            raise HomeAssistantError(msg)
        if (
            self.feeder.battery.percentage < 10
            and not self.feeder.battery.is_charging
        ):
            msg = (
    ...
    async def async_install(
        ...
        try:
            result = await self.coordinator.client.update_firmware_start(
                self.feeder
            )
        ...
        while not result.is_complete:
            ...
            try:
                ...
                if errors >= MAX_ERRORS:
                    ...
                    raise
                LOGGER.warning(
                    "Error checking update progress; "
                    "will try again (%d/%d): %s",
                    errors,
                    MAX_ERRORS,
                    exc,
                )
