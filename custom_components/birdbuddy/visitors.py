...
from .const import CONF_FEEDER_ID, EVENT_NEW_POSTCARD, LOGGER
...
class RecentVisitors:
    ...
    def _stop(self) -> None:
        ...
        LOGGER.info(
            "Stopped listening for new visitors to feeder %s", self.feeder.name
        )
    ...
    def _start(self) -> Callable[[], None]:
        ...
        def filter_my_postcards(event_data: Mapping[str, Any]) -> bool:
            ...
            """
            return self.feeder.id == event_data.get(CONF_FEEDER_ID)
    ...
    async def _on_new_postcard(self, event: Event) -> None:
        ...
        """
        media = event.data.get("media")
        species = event.data.get("species") or []
