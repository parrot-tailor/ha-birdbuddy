...
def _feeder(*, owner=True, **extra: object):
    ...
    return BirdBuddyDevice(
        {"__typename": typename, "id": "f1", "name": "BB", **extra}
    )
...
async def test_offgrid_turn_on_updates_and_notifies(hass):
    ...
    coordinator.client.toggle_off_grid = AsyncMock(
        return_value={"offGrid": True}
    )
